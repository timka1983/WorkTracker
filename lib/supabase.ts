  ...
  const checkSession = async () => {
    try {
      ...
      if (error && (error.message.includes('Refresh Token Not Found') || error.message.includes('invalid_grant'))) {
        ...
        // Clear all Supabase related items from localStorage just in case
...
console.log('Supabase initialized:', checkConfig());
    ...
    try {
      ...
      for (const col of columns) {
        try {
          ...
          if (error) {
            console.warn(`Failed to add column ${col.column} to ${col.table} via RPC:`, error);
            anyError = true;
          }
        } catch (e) {
          console.warn(`Exception adding column ${col.column} to ${col.table}:`, e);
          anyError = true;
        }
      }
...
ALTER TABLE organizations ADD COLUMN IF NOT EXISTS plan TEXT DEFAULT 'FREE';`
  ...
  checkPin: async (userId: string, pin: string) => {
    if (!checkConfig()) return false;
    try {
      ...
      console.error('Exception checking PIN:', e);
      return false;
    }
  },
  ...
  getUsers: async (orgId: string): Promise<User[] | null> => {
    if (!checkConfig()) {
      console.error('Supabase is not configured. Please check VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY.');
      return null;
    }
    try {
      ...
      if (error) {
        // Fallback for missing columns (42703, PGRST204)
        if (error.code === '42703' || error.code === 'PGRST204' || error.message?.includes('column')) {
          console.warn('Some columns missing in users table, retrying with minimal set');
          ...
          if (minimalError) {
            // Even organization_id might be missing
            if (minimalError.code === '42703' || minimalError.message?.includes('organization_id')) {
               const { data: allData, error: allError } = await supabase.from('users').select('id, name, role, position, pin').order('name');
               if (allError) throw allError;
               return (allData || []).map(u => ({
                 ...
                 position: u.position,
                 pin: u.pin,
                 isAdmin: u.id === 'admin' || u.position === 'Администратор',
  ...
  upsertUser: async (user: any, orgId: string) => {
    ...
    const payload: any = {
      ...
      pin: user.pin,
      email: user.email,
      pin_hash: user.pin ? (user.pin.startsWith('$2a$') || user.pin.startsWith('$2b$') ? user.pin : await bcrypt.hash(user.pin, 10)) : null,
    ...
    console.log('Upserting user with payload:', { ...payload, pin_hash: '[REDACTED]' });
