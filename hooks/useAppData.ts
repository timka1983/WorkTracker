...
export const useAppData = (currentUser: User | null) => {
  ...
  const [orgId, setOrgId] = useState<string>(() => {
    ...
    // Auto-migrate 'demo_org' to 'initial_org' to fix visibility issues
    if (stored === 'demo_org') {
      console.log('⚠️ Migrating from demo_org to initial_org');
      stored = 'initial_org';
      localStorage.setItem(STORAGE_KEYS.ORG_ID, 'initial_org');
    }
  ...
  const addUserMutation = useMutation({
    mutationFn: async (user: User) => {
      ...
      // Use the new RPC function for automated employee creation
  ...
  const handleDeleteBranch = async (branchId: string) => {
    const confirmed = await confirm({
      ...
      message: 'Вы уверены? Удаление филиала не удалит сотрудников и логи, но отвяжет их.',
      type: 'danger'
    });
    if (confirmed) {
  ...
  const handleCleanupDatabase = async () => {
    ...
    const confirmed = await confirm({
      title: 'Очистка базы',
      message: 'Это действие объединит дубликаты сотрудников (например, admin и $admin) и исправит ссылки в логах. Продолжить?',
      type: 'warning'
    });
    if (confirmed) {
  ...
  const handleRemoveBase64Photos = async () => {
    if (!currentOrg) return;
    const confirmed = await confirm({
      title: 'Удаление фото Base64',
      message: 'Это действие удалит все фото, сохраненные в формате Base64 (внутри базы), чтобы снизить нагрузку. Фото в хранилище останутся. Продолжить?',
      type: 'warning'
    });
    if (confirmed) {
  ...
  const handleMergeDuplicates = async () => {
    ...
    const confirmed = await confirm({
      ...
      message: 'ВНИМАНИЕ: Это действие объединит всех сотрудников и СТАНКИ с одинаковыми именами. Логи будут перенесены на "основные" записи, остальные дубликаты будут удалены. Это действие необратимо. Продолжить?',
      type: 'danger'
    });
    if (confirmed) {
