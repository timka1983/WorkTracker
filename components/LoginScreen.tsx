...
interface LoginScreenProps {
  ...
  validateAndLogin: (pin: string, user?: User) => Promise<void>;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl dark:shadow-slate-900/40 border border-slate-200 dark:border-slate-800 p-8 w-full max-w-md relative overflow-hidden">
                ...
                <button
                  ...
                  onClick={async () => {
                    if (n === 'del') {
                      ...
                      setPinInput(newPin);
                      if (newPin.length === 4) {
                        await validateAndLogin(newPin, selectedLoginUser);
                      }
                    }
                  }}
        ...
        <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800 flex justify-center">
          <a href="/admin-login" className="text-[10px] text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 uppercase font-black tracking-widest transition-colors">Вход для администратора</a>
        </div>
      </div>
    </div>
  );
